...
work_directory <- "/Users/tiagofalcon/Desktop/cristal/16s_files_to_run" # Path to your work directory. This is 
# the directory where you may find the two necessary files to begin the analysis and where you gonna save the outputs.
file_name <- "111906_counts.tsv" # Change the file name. It is a tab delimited OTU table.
...
####Compounds/genes### level
compound_or_gene_no_unclassified <- subset(input,!(input$Level=="Unclassified"))
compound_or_gene_no_unclassified_edt <-cbind (compound_or_gene_no_unclassified$Level, compound_or_gene_no_unclassified[,!lapply(compound_or_gene_no_unclassified, class) == "factor"])
# One row per taxon...
combine__rows_compound_or_gene_m <- aggregate(compound_or_gene_no_unclassified_edt[,-1], by = list(compound_or_gene_no_unclassified_edt[,1]), FUN = sum)
...
numbers_to_norm <- combine__rows_compound_or_gene_m[,!lapply(combine__rows_compound_or_gene_m, class) == "factor"]
...
end_file_compound_or_gene <- cbind (combine__rows_compound_or_gene_m[,lapply(combine__rows_compound_or_gene_m, class) == "factor"], transformed_df)
# Save the transformed output file in case you need it later.
write.table (end_file_compound_or_gene, 'compound_or_gene_log_ratio_transformed_data.txt', sep = '\t', row.names = FALSE)
#
#
#
# Create a barplot with top five taxa based on the first group ...
combine__rows_compound_or_gene_m2 <- t(combine__rows_compound_or_gene_m)
colnames(combine__rows_compound_or_gene_m2)<- combine__rows_compound_or_gene_m2[1,]
combine__rows_compound_or_gene_m2 <- data.frame(combine__rows_compound_or_gene_m2)
combine__rows_compound_or_gene_m2 <- combine__rows_compound_or_gene_m2[-1,]
combine__rows_compound_or_gene_m2[, ncol(combine__rows_compound_or_gene_m2)+1] <- NA
for (i in levels(map_groups$Description)){
  data_i <- as.vector(subset(map_groups, Description == i)[,1])
  for (name_col in data_i) {
    combine__rows_compound_or_gene_m2[which(row.names(combine__rows_compound_or_gene_m2) == name_col), ncol(combine__rows_compound_or_gene_m2)] <- i
  }
}
colnames(combine__rows_compound_or_gene_m2)[ncol(combine__rows_compound_or_gene_m2)] <- c("group_ID")
for (i in 1:(ncol(combine__rows_compound_or_gene_m2)-1)){
  combine__rows_compound_or_gene_m2[,i] <- as.numeric(as.character(combine__rows_compound_or_gene_m2[,i]))
}
combine__rows_compound_or_gene_m2 <- aggregate(combine__rows_compound_or_gene_m2[,-ncol(combine__rows_compound_or_gene_m2)], by = list(combine__rows_compound_or_gene_m2[,ncol(combine__rows_compound_or_gene_m2)]), FUN = mean)
t_combine__rows_compound_or_gene_m2 <- t(combine__rows_compound_or_gene_m2)
colnames(t_combine__rows_compound_or_gene_m2) <- t_combine__rows_compound_or_gene_m2[1,]
t_combine__rows_compound_or_gene_m2 <- t_combine__rows_compound_or_gene_m2[-1,]
t_combine__rows_compound_or_gene_m2 <- data.frame(t_combine__rows_compound_or_gene_m2)
for (i in 1:ncol(t_combine__rows_compound_or_gene_m2)){
  t_combine__rows_compound_or_gene_m2[,i] <- as.numeric(as.character(t_combine__rows_compound_or_gene_m2[,i]))
}
prop_compound_or_gene_data <- t(t(t_combine__rows_compound_or_gene_m2)/rowSums(t(t_combine__rows_compound_or_gene_m2)))
colSums(prop_compound_or_gene_data)
prop_compound_or_gene_data <- prop_compound_or_gene_data[order(prop_compound_or_gene_data[,1], decreasing = TRUE),]
pdf('barplot_top5_compound_or_gene.pdf')
par(xpd = NA)
barplot(prop_compound_or_gene_data[1:5,],main = "Top 5 most representative Compound or gene",
...
legend("topright", row.names(prop_compound_or_gene_data[1:5,]), cex=0.5, fill = color_data[1:5])
dev.off()
#
#
#
...
transposed_taxa <- t(end_file_compound_or_gene)
...
merged_df <- merge(map_groups, transposed_taxa, by.x='Sample_ID', by.y= 'row.names')
...
if (length(levels(transposed_taxa$Description)) != 2){
  warning("You don't have two groups")} else if (exists('data_ttest_sum') == TRUE){
    ...
    write.table(data_ttest_sum, file = "comparisons_compound_or_gene_t_test.txt", append = FALSE, row.names = FALSE, col.names = TRUE, sep='\t')
  }
if (length(levels(transposed_taxa$Description)) != 2){
  warning("You don't have two groups")} else if (exists('data_wil_sum') == TRUE){
    data_wil_sum[,3] <- p.adjust(data_wil_sum[,3], method = 'fdr')
    write.table(data_wil_sum, file = "comparisons_compound_or_gene_wilcox.txt", append = FALSE, row.names = FALSE, col.names = TRUE, sep='\t')
  }
#
#
#
...
trans_data <- cbind (combine__rows_compound_or_gene_m[,lapply(combine__rows_compound_or_gene_m, class) == "factor"], transformed_df)
trans_data<-na.omit(trans_data)
trans_data$CntlCountMean<- (10**rowMeans(trans_data[, 2:4]))
trans_data$MPSCountMean<- (10**rowMeans(trans_data[, 5:13]))
trans_data$MPSlog2<-log(abs(trans_data$MPSCountMean),base = 2)
trans_data$Cntllog2<-log(abs(trans_data$CntlCountMean),base = 2)
trans_data$FClog2<- (trans_data$MPSlog2-trans_data$Cntllog2)
write.csv (trans_data, '111906_log2FC')
